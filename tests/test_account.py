...
from nempy import user_data


@patch.object(AccountData, 'input_keyprint_entropy', return_value='test')
def test_account(mock_keyprint_entropy):
    network_type = NetworkType.TEST_NET
    bip32_coin_id_test_net = 1

    with patch.object(AccountData, 'inquirer_account', return_value='TD6HYHFAQPGN3QB5GH46I35RYWM5VMMBOW32RXQ'):
        account0 = AccountData.account_by_mnemonic(network_type=network_type, bip32_coin_id=bip32_coin_id_test_net)
        with patch.object(stdiomask, 'getpass', return_value=account0.mnemonic):
            account0_ = AccountData.account_by_mnemonic(network_type=network_type, bip32_coin_id=bip32_coin_id_test_net,
                                                        is_import=True)
            assert account0.address == account0_.address

    with patch.object(AccountData, 'inquirer_account', return_value='TBR5X6UG3ZT2IIOAP65Y7J7SLPN4UPARKH6HMUI'):
        account1 = AccountData.account_by_mnemonic(network_type=network_type, bip32_coin_id=bip32_coin_id_test_net)
...
def test_get_generation_type():
    with patch('inquirer.prompt', return_value={'type': 'Private Key'}):
        assert GenerationType.PRIVATE_KEY == AccountData.get_generation_type()

    with patch('inquirer.prompt', return_value={'type': 'Mnemonic'}):
        assert GenerationType.MNEMONIC == AccountData.get_generation_type()


def test_inquirer_account():
    with patch('inquirer.prompt', return_value={'address': 'Address2'}):
        assert 'Address2' == AccountData.inquirer_account(['Address1', 'Address2'])


def test_input_keyprint_entropy():
    value = 'test'
    with patch('nempy.account.input', side_effect=[value, '', value, value, value, value]):
        assert value * 3 in AccountData.input_keyprint_entropy()
...
def test_init_general_params():
    with tempfile.TemporaryDirectory() as accounts_dir:
        ...
        with patch('nempy.account.input', side_effect=['', 'test_exist', 'test', 'y']):
            ...
            account_path, name, bip32_coin_id, is_default = AccountData.init_general_params(NetworkType.TEST_NET, accounts_dir)
        ...
        with patch('nempy.account.input', side_effect=['test', 'y']):
            account_path, name, bip32_coin_id, is_default = AccountData.init_general_params(NetworkType.MAIN_NET, accounts_dir)
            assert account_path == os.path.join(accounts_dir, tmp_test_account)
        ...
        with patch('nempy.account.input', side_effect=['test', 'n']):
            with pytest.raises(ValueError):
                AccountData.init_general_params('MAIN_NET', accounts_dir)
...
def test_encryption_decryption():
    data = b'data'
    password = 'pass'
    enc_data = user_data.encryption(password, data)
    _data = user_data.decryption(password, enc_data)
    assert data == _data
    assert user_data.decryption(password + 'random', enc_data) is None
...
class TestAccount:
    ...
    def setup(self, mock_ike):
        ...
        with patch.object(AccountData, 'inquirer_account', return_value='TD6HYHFAQPGN3QB5GH46I35RYWM5VMMBOW32RXQ'):
            self.account = AccountData.account_by_mnemonic(network_type=network_type, bip32_coin_id=bip32_coin_id_test_net)
    ...
    def test_write_read(self):
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            assert user_data.DecoderStatus.NO_DATA == _account
